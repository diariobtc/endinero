...
fn integer_part_tests() {
    ...
    assert_eq!(integer_part_f64
                   (1234567890123.0, '.'), "1.234.567.890.123");
    assert_eq!(integer_part_f64
                   (-1234567890123.0, '.'), "-1.234.567.890.123");

    info!("Tens of trillions (10^13) / Decenas de billones");
    assert_eq!(integer_part_f64
                   (12345678901234.0, '.'), "12.345.678.901.234");
    assert_eq!(integer_part_f64
                   (-12345678901234.0, '.'), "-12.345.678.901.234");

    info!("Hundreds of trillions (10^14) / Cientos de billones");
    assert_eq!(integer_part_f64
                   (123456789012345.0, '.'), "123.456.789.012.345");
    assert_eq!(
        integer_part_f64
            (-123456789012345.0, '.'),
        "-123.456.789.012.345"
...
fn decimal_part_tests() {
    ...
    assert_eq!(decimal_part_f64(0.2223334445556, 12, '.'), "222.333.444.555");
    assert_eq!(decimal_part_f64(0.2223334445556, 13, '.'), "222.333.444.555.6");
}

#[test]
fn tests() {
    env_logger::try_init();
    assert_eq!(endinero_f64(1234567.456, 2, 4, '.', ',', ' '), "1.234.567,45");
    assert_eq!(
    ...
    let x = 1000000.1232456 as f32;
    assert_eq!(dinero_f64(x as f64), "1.000.000,12");
}
